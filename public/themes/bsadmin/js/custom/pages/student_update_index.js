    ...
    if ( data.id > 0 || data.name != "" ) {
        ...
        formPost(form).then(json => {
            if (json.status == 'error') {
                ...
                resp.close  = false
                resp.title  = `<i class="fa fa-file-text-o fa-wd"></i> Update student's register`
                resp.body   = `The register's information has been updated successfully`
                resp.footer = `<a href="${AdminPath}/schools" class="btn btn-default pull-left">Back to students</a>
    ...
    if ( data.id > 0 ) {
        ...
        jsonPost(form).then(json => {
            if (json.status == 'error') {
                ...
                document.querySelector('.modal-footer').innerHTML = `<a href="${AdminPath}/school/students?id=${return_id}" class="btn btn-success"><i class="fa fa-check fa-fw"></i>  Back to list</a>`
...
deleteRegisterDialogue = (id) => {
    ...
    resp.body   = `Are you sure to delete <b>${inputName.value} ${inputSurname.value}</b>'s student register?`
...
dialogueImageNull = () => {
    ...
    resp.body   = `Are you sure to update this student register without profile image?`
    resp.footer =  `<button type="button" class="btn btn-default pull-left" data-dismiss="modal">No, close</button>
